...
type SSHClientInterface interface {
	...
	UploadWithRetryProgress(localPath, remotePath string, progressCallback func(uploaded, total int64)) error
...
func (c *SSHClient) EnableAutoAdd(enabled bool) {
	c.hostKeyChecker.EnableAutoAdd(enabled)
}
...
func (c *SSHClient) UploadWithProgress(localPath, remotePath string, progressCallback func(uploaded, total int64)) error {
	...
	if offset == 0 {
		...
		remoteFile, err = c.sftpClient.OpenFile(remotePartPath, os.O_WRONLY)
		if err == nil {
			_, err = remoteFile.Seek(offset, io.SeekStart)
			if err != nil {
				remoteFile.Close()
				logger.Get().Warn().
					Str("file", localPath).
					Msg("Cannot seek in part file, restarting upload")
				c.sftpClient.Remove(remotePartPath)
				remoteFile, err = c.sftpClient.Create(remotePartPath)
				offset = 0
			}
...
func (c *SSHClient) SetKnownHostsPath(path string) {
	c.hostKeyChecker = NewHostKeyChecker(path)
}
